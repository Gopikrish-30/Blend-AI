...
const api = {
  ...
  blender: {
    getConnectionStatus: (): Promise<{
      ...
      port: number
      checkedAt: number
    }> => ipcRenderer.invoke(IpcChannel.Blender_GetConnectionStatus),
    checkConnection: (): Promise<{
      status: 'connected' | 'disconnected'
      host: string
      port: number
      checkedAt: number
    }> => ipcRenderer.invoke(IpcChannel.Blender_CheckConnection),
    executeCommand: (
      type: string,
      params?: Record<string, unknown>
    ): Promise<{ success: boolean; result?: Record<string, unknown>; error?: string }> =>
      ipcRenderer.invoke(IpcChannel.Blender_ExecuteCommand, { type, params }),
    onConnectionStatusChanged: (
      callback: (state: { status: 'connected' | 'disconnected'; host: string; port: number; checkedAt: number }) => void
    ): (() => void) => {
      const channel = IpcChannel.Blender_ConnectionStatusChanged
      const listener = (_: Electron.IpcRendererEvent, state: Parameters<typeof callback>[0]) => callback(state)
      ipcRenderer.on(channel, listener)
