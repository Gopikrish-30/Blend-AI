...
import { BlenderAssetPreview, isBlenderAssetTool } from './BlenderAssetPreview'
...
export function ToolPermissionRequestCard({ toolResponse }: Props) {
  ...
  const toolName = toolResponse.tool?.name ?? ''
  const toolInput = (approval.input ?? toolResponse.arguments) as Record<string, unknown> | undefined

  return (
    <Container>
      {/* Blender asset visual preview — only for asset download/generation tools */}
      {isBlenderAssetTool(toolName) && toolInput && (
        <>
          <BlenderAssetPreview toolName={toolName} input={toolInput} />
          <Divider />
        </>
      )}
