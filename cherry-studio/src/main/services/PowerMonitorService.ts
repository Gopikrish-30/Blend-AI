...
try {
  ...
  if (ElectronShutdownHandler && ElectronShutdownHandler.default) {
    ElectronShutdownHandler = ElectronShutdownHandler.default
  }
} catch (e) {
  logger.warn('Failed to load @paymoapp/electron-shutdown-handler, using fallback stub')
