...
export async function resolveAssistantMcpToolIds(assistantId: string): Promise<string[]> {
  ...
  return resolveToolIdsForServers(servers)
}

/**
 * Resolve tool IDs for all active MCP servers, regardless of assistant.
 * Used as a fallback when no assistantId is set on the request — the user
 * is in a plain chat topic without an attached assistant, so we treat all
 * active servers as available (equivalent to mcpMode: 'auto').
 */
export async function resolveAllActiveMcpToolIds(): Promise<string[]> {
  const { items: activeServers } = await mcpServerService.list({ isActive: true })
  if (activeServers.length === 0) return []
  return resolveToolIdsForServers(activeServers)
}

async function resolveToolIdsForServers(servers: McpServer[]): Promise<string[]> {
