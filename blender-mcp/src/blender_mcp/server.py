...
def set_object_transform(ctx: Context, name: str, location: Optional[list] = None, rotation: Optional[list] = None, scale: Optional[Union[float, list]] = None, user_prompt: str = "") -> str:
    """
    Set the location, rotation (Euler in radians), and/or scale of a named object.

    Parameters:
    - name: Object name
    - location: [X, Y, Z] in metres
    - rotation: [X, Y, Z] in radians (Euler)
    - scale: uniform float OR [X, Y, Z]
...
def create_primitive(ctx: Context, primitive_type: str, name: Optional[str] = None, location: Optional[list] = None, scale: Optional[Union[float, list]] = None, user_prompt: str = "") -> str:
    """
...
def delete_objects(ctx: Context, names, user_prompt: str = "") -> str:
    """
    Delete one or more objects by name.

    Parameters:
    - names: single object name string OR list of names
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("delete_objects", {"names": names})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
def duplicate_object(ctx: Context, name: str, new_name: Optional[str] = None, location_offset: Optional[list] = None, linked: bool = False, user_prompt: str = "") -> str:
    """
    Duplicate an object. Optionally rename and offset the duplicate's location.

    Parameters:
    - name: source object name
    - new_name: name for the duplicate
    - location_offset: [dX, dY, dZ] offset applied to the duplicate
    - linked: if True, data is shared (linked duplicate)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
...
@mcp.tool()
@telemetry_tool("set_object_visibility")
def set_object_visibility(ctx: Context, name: str, hide: bool = False, hide_render: Optional[bool] = None, user_prompt: str = "") -> str:
    """
    Show or hide an object in the viewport and optionally in renders.

    Parameters:
    - name: object name
    - hide: True to hide in viewport, False to show
    - hide_render: True to exclude from render (optional)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        params = {"name": name, "hide": hide}
...
def set_origin(ctx: Context, name: str, origin_type: str = "ORIGIN_GEOMETRY", user_prompt: str = "") -> str:
    """
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("set_origin", {"name": name, "origin_type": origin_type})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
def apply_transform(ctx: Context, name: str, apply_location: bool = False, apply_rotation: bool = True, apply_scale: bool = True, user_prompt: str = "") -> str:
    """
    Apply (bake) location/rotation/scale transforms into the object's mesh data.

    Parameters:
    - name: object name
    - apply_location: bake location (default False)
    - apply_rotation: bake rotation (default True)
    - apply_scale: bake scale (default True)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("apply_transform", {
            "name": name, "apply_location": apply_location,
            "apply_rotation": apply_rotation, "apply_scale": apply_scale
        })
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
def parent_object(ctx: Context, child_name: str, parent_name: str, keep_transform: bool = True, user_prompt: str = "") -> str:
    """
    Set a parent-child relationship between two objects.

    Parameters:
    - child_name: the object that will become the child
    - parent_name: the object that will be the parent
    - keep_transform: preserve world position of child (default True)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("parent_object", {
            "child_name": child_name, "parent_name": parent_name, "keep_transform": keep_transform
        })
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
def snap_to_ground(ctx: Context, name: str, ground_z: float = 0.0, user_prompt: str = "") -> str:
    """
    Move an object so its lowest bounding-box point sits exactly at ground_z (default 0).
    Essential for correct floor placement of furniture and props.

    Parameters:
    - name: object name
    - ground_z: target Z for the bottom of the object (default 0.0)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
...
@mcp.tool()
@telemetry_tool("align_objects")
def align_objects(ctx: Context, names: list, axis: str = "X", align_to: str = "MIN", user_prompt: str = "") -> str:
    """
    ...
    - axis: X, Y, or Z
    - align_to: MIN (leftmost), MAX (rightmost), CENTER (average)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("align_objects", {"names": names, "axis": axis, "align_to": align_to})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
def create_collection(ctx: Context, name: str, parent_collection: Optional[str] = None, user_prompt: str = "") -> str:
    """
    Create a new collection for scene organisation.

    Parameters:
    - name: collection name
    - parent_collection: optional parent collection name
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
...
def move_to_collection(ctx: Context, object_names, collection_name: str, user_prompt: str = "") -> str:
    """
    Move objects into a named collection (creates the collection if it does not exist).

    Parameters:
    - object_names: single name or list of names
    - collection_name: target collection name
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("move_to_collection", {"object_names": object_names, "collection_name": collection_name})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("list_collections")
def list_collections(ctx: Context, user_prompt: str = "") -> str:
    """
    List all collections in the scene with their contained object names and child collections.
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("list_collections", {})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("set_active_object")
def set_active_object(ctx: Context, name: str, user_prompt: str = "") -> str:
    """
    Deselect all objects, then select and make a single object active.
...
def create_material(ctx: Context, name: str, base_color: Optional[list] = None, roughness: float = 0.5, metallic: float = 0.0, alpha: float = 1.0, user_prompt: str = "") -> str:
    """
    ...
    try:
        blender = get_blender_connection()
        params = {"name": name, "roughness": roughness, "metallic": metallic, "alpha": alpha}
...
def assign_material(ctx: Context, object_name: str, material_name: str, slot: int = 0, user_prompt: str = "") -> str:
    """
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("assign_material", {"object_name": object_name, "material_name": material_name, "slot": slot})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("set_material_property")
def set_material_property(ctx: Context, material_name: str, property_name: str, value, user_prompt: str = "") -> str:
    """
    Set a Principled BSDF input value on a material.

    Parameters:
    - material_name: name of the material
    - property_name: e.g. 'Base Color', 'Roughness', 'Metallic', 'Emission Color', 'Emission Strength', 'Alpha', 'IOR'
    - value: float for scalars, [R,G,B] or [R,G,B,A] for colors
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("set_material_property", {"material_name": material_name, "property_name": property_name, "value": value})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
def list_materials(ctx: Context, user_prompt: str = "") -> str:
    """
...
@mcp.tool()
@telemetry_tool("get_material_info")
def get_material_info(ctx: Context, material_name: str, user_prompt: str = "") -> str:
    """
    Get detailed info about a material including all Principled BSDF property values.
...
def set_world_hdri(ctx: Context, hdri_path: Optional[str] = None, strength: float = 1.0, color: Optional[list] = None, user_prompt: str = "") -> str:
    """
    Set the world environment — either an HDRI image file or a solid background colour.

    Parameters:
    - hdri_path: absolute path to an .hdr or .exr file (from PolyHaven download)
    - strength: environment strength multiplier
    - color: [R, G, B] background colour (used when no hdri_path given)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
...
def create_light(ctx: Context, light_type: str, name: Optional[str] = None, location: Optional[list] = None, energy: float = 10.0, color: Optional[list] = None, radius: float = 0.25, user_prompt: str = "") -> str:
    """
    ...
    - name: optional name
    - location: [X, Y, Z]
    - energy: light strength in watts
    - color: [R, G, B] light colour
    - radius: shadow softness radius (POINT/SPOT) or area size (AREA)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        params = {"light_type": light_type, "energy": energy, "radius": radius}
        if name:
            params["name"] = name
        if location:
            params["location"] = location
...
@mcp.tool()
@telemetry_tool("set_light_property")
def set_light_property(ctx: Context, name: str, energy: Optional[float] = None, color: Optional[list] = None, radius: Optional[float] = None, angle: Optional[float] = None, user_prompt: str = "") -> str:
    """
    Adjust properties of an existing light.

    Parameters:
    - name: light object name
    - energy: new energy value
    - color: [R, G, B]
    - radius: shadow softness / area size
    - angle: spot angle in degrees (SPOT only)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
...
def list_lights(ctx: Context, user_prompt: str = "") -> str:
    """
    List all lights in the scene with type, energy, colour, and location.
...
def create_camera(ctx: Context, name: Optional[str] = None, location: Optional[list] = None, rotation: Optional[list] = None, focal_length: float = 50.0, user_prompt: str = "") -> str:
    """
...
@mcp.tool()
@telemetry_tool("set_camera_property")
def set_camera_property(ctx: Context, name: str, focal_length: Optional[float] = None, sensor_width: Optional[float] = None, dof_distance: Optional[float] = None, dof_enabled: Optional[bool] = None, user_prompt: str = "") -> str:
    """
    Set properties on a camera (focal length, sensor width, depth of field).

    Parameters:
    - name: camera object name
    - focal_length: in mm
    - sensor_width: in mm
    - dof_distance: depth-of-field focus distance in metres
    ...
    try:
        ...
        if dof_distance is not None:
            params["dof_distance"] = dof_distance
...
@mcp.tool()
@telemetry_tool("frame_objects")
def frame_objects(ctx: Context, names: Optional[list] = None, user_prompt: str = "") -> str:
    """
    Frame specified objects (or all objects) in the 3D viewport.
    Call after placing or moving objects to centre the view on them.

    Parameters:
    - names: list of object names (omit or pass null for all objects)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        ...
        if names:
            params["names"] = names
        result = blender.send_command("frame_objects", params)
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
def add_modifier(ctx: Context, object_name: str, modifier_type: str, name: Optional[str] = None, user_prompt: str = "", **kwargs) -> str:
    """
    ...
    Extra kwargs are forwarded to the modifier (e.g. levels=2 for SUBSURF, thickness=0.05 for SOLIDIFY).
    """
    try:
        blender = get_blender_connection()
...
def apply_modifier(ctx: Context, object_name: str, modifier_name: str, user_prompt: str = "") -> str:
    """
    Apply (bake) a modifier on an object by modifier name.
...
def list_modifiers(ctx: Context, object_name: str, user_prompt: str = "") -> str:
    """
...
def set_render_settings(ctx: Context, engine: Optional[str] = None, resolution_x: Optional[int] = None, resolution_y: Optional[int] = None, samples: Optional[int] = None, film_transparent: Optional[bool] = None, user_prompt: str = "") -> str:
    """
    ...
    - samples: render samples (Cycles quality / EEVEE AA)
    - film_transparent: enable transparent background
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
...
@mcp.tool()
@telemetry_tool("get_render_settings")
def get_render_settings(ctx: Context, user_prompt: str = "") -> str:
    """
    Get the current render engine, resolution, and sample settings.
...
# =============================================================================
# Verification / QA Tools  (agent self-check)
# =============================================================================

@mcp.tool()
@telemetry_tool("get_scene_statistics")
def get_scene_statistics(ctx: Context, user_prompt: str = "") -> str:
    """
    Get comprehensive scene statistics: object counts, polygon/vertex counts, lights, cameras, materials.
    Use this to verify scene complexity and asset counts after building.
...
@mcp.tool()
@telemetry_tool("check_object_placement")
def check_object_placement(ctx: Context, names: Optional[list] = None, user_prompt: str = "") -> str:
    """
    Return world-space bounding boxes of objects and detect AABB intersections.
    Use this after placing furniture/props to verify correct positions and no unwanted clipping.

    Parameters:
    - names: list of object names to check (omit for all mesh objects)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        ...
        if names:
            params["names"] = names
        result = blender.send_command("check_object_placement", params)
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("get_world_settings")
def get_world_settings(ctx: Context, user_prompt: str = "") -> str:
    """
    Return current world/environment node settings: HDRI file, background colour, strength.
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("get_world_settings", {})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("list_objects_by_type")
def list_objects_by_type(ctx: Context, obj_type: Optional[str] = None, user_prompt: str = "") -> str:
    """
    List all scene objects, optionally filtered by type.
    Faster than get_scene_info for targeted queries.

    Parameters:
    - obj_type: MESH, LIGHT, CAMERA, CURVE, ARMATURE, EMPTY (omit for all)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        ...
        result = blender.send_command("list_objects_by_type", params)
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("validate_scene")
def validate_scene(ctx: Context, user_prompt: str = "") -> str:
    """
    Run a scene health check: detects zero-scale objects, empty material slots, missing textures,
    no active camera, no lights. Returns a list of issues and suggestions.
    Always call this before reporting a scene as finished.
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("validate_scene", {})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
# =============================================================================
# Spatial Intelligence / Planning Tools
# =============================================================================

@mcp.tool()
@telemetry_tool("get_scene_bounds")
def get_scene_bounds(ctx: Context, user_prompt: str = "") -> str:
    """
    Get the overall world-space bounding box of all mesh objects in the scene.
    Use at the start of any task to understand how much space is already occupied
    and where free space is available.

    Returns: min/max corners, size (WxDxH), and center of the scene footprint.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("get_scene_bounds", {})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("get_floor_plan")
def get_floor_plan(ctx: Context, user_prompt: str = "") -> str:
    """
    Get a 2D top-down floor plan of all mesh objects: X/Y position, footprint width/depth,
    height, and bounding box extents.

    Use this BEFORE placing any object to:
    - See exactly where everything is on the floor
    - Find free floor areas for new objects
    - Decide placement coordinates intelligently
    - Avoid clipping and spacing errors

    Returns per-object: center_xy, footprint_wh, bbox_min_xy, bbox_max_xy, bottom_z, top_z.
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("get_floor_plan", {})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("measure_distance")
def measure_distance(ctx: Context, object_a: str, object_b: str, user_prompt: str = "") -> str:
    """
    Measure the centre-to-centre distance and the minimum bounding-box gap between two objects.
    Use to verify spacing (e.g. 0.6m walkway between sofa and coffee table).
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("measure_distance", {"object_a": object_a, "object_b": object_b})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
...
@mcp.tool()
@telemetry_tool("suggest_placement")
def suggest_placement(ctx: Context, object_width: float, object_depth: float, ground_z: float = 0.0, margin: float = 0.15, user_prompt: str = "") -> str:
    """
    Given an object's footprint (width × depth in metres), return up to 8 candidate floor
    positions that don't overlap any existing object (with a clearance margin).

    Use this when you need to place a new object but are unsure of coordinates.
    Always call get_floor_plan first to understand what's already placed.

    Parameters:
    - object_width: object X dimension in metres
    - object_depth: object Y dimension in metres
    - ground_z: floor Z level (default 0.0)
    - margin: clearance gap between objects in metres (default 0.15)
    - user_prompt: original user prompt (telemetry)
    """
    try:
        blender = get_blender_connection()
        result = blender.send_command("suggest_placement", {
...
@mcp.tool()
@telemetry_tool("get_full_scene_context")
def get_full_scene_context(ctx: Context, user_prompt: str = "") -> str:
    """
    **PLANNING TOOL** — Call this at the start of any complex task (scene creation,
    furniture layout, lighting setup, etc.) before writing a plan.

    Returns a comprehensive snapshot of the current Blender scene:
    - All objects with locations and types
    - 2D floor plan (footprints, positions)
    - Scene bounds (total occupied space)
    - Statistics (poly count, material count)
    - World / HDRI settings
    ...
    After calling this, use the data to draft a detailed task plan for the user.
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("get_full_scene_context", {})
        return json.dumps(result, indent=2)
    except Exception as e:
        return f"Error: {str(e)}"
